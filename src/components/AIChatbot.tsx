...
const LIMIT_MESSAGES = {
    EN: "You have reached the free AI consultation limit (5/5). To receive personalized medical care, please [**Book an Appointment**](/book) or call us directly at **(808) 528-7177**.",
    JA: "無料AI相談の制限（5/5）に達しました。個別のご相談については、[**ご予約**](/book)いただくか、**(808) 528-7177**までお電話ください。",
    ZH: "您已达到免费AI咨询次数上限 (5/5)。如需获得个性化的专业医疗建议，请[**在线预约**](/book) 或直接致电 **(808) 528-7177**。",
    KO: "무료 AI 상담 한도(5/5)에 도달했습니다. 개별적인 전문 상담을 원하시면 [**예약하기**](/book)를 이용하시거나 **(808) 528-7177**로 직접 전화해 주십시오.",
    ES: "Ha alcanzado el límite de consultas gratuitas de IA (5/5). Para recibir atención médica personalizada, por favor [**Reserve una Cita**](/book) o llámenos directamente al **(808) 528-7177**."
};
...
export default function AIChatbot() {
    ...
    // Initialize usage count and monitor language changes
    useEffect(() => {
        ...
        if (count >= 5) {
            setMessages(prev => {
                ...
                // Avoid duplicating the limit message if it exists
                ...
                <div className="mb-4 w-80 rounded-2xl border border-zinc-200 bg-white shadow-xl dark:border-zinc-800 dark:bg-zinc-900 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300 h-96">
                    ...
                    <form onSubmit={handleSend} className="p-3 bg-white dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 flex gap-2">
                        <input
                            ...
                            className="flex-1 rounded-full border border-zinc-300 px-4 py-2 text-sm focus:border-gold focus:outline-none dark:bg-zinc-800 dark:border-zinc-700 disabled:opacity-50 disabled:bg-zinc-100 dark:disabled:bg-zinc-800"
                        />
                        <button
                            type="submit"
                            className="rounded-full bg-navy p-2 text-white hover:bg-navy/90 disabled:opacity-50 dark:disabled:bg-zinc-700"
